...
;; indentation problem with "blocks within blocks".  Thanks to Jйrome
	...
	(concat
	 ...
	 (regexp-opt
	  (list
	   ...
	   "go"
	   ;;	 "goto"
	   "grant" "group" "having" "identified" "immediate"
	   ;;;;"in"
	   "increment"
	   "index"
	   ;;       "indicator"
    ...
    ;(unless (and  plsql-align-rules-list plsql-debug)
      (setq plsql-align-rules-list
	    '(
	      ...
	      (plsql-keywords
	       (regexp . "\\(\\s-+\\)\\(in\\|default\\|constant\\|number\\|char\\|varchar2\\(([1234567890]+)\\)\\|blob\\|raw\\)\\b")
	       ;;(regexp . "\\(\\s-+\\)\\(in\\|default\\|number\\|char\\|varchar2\\|blob\\|raw\\)\\b")
	       (group 1)
	       (repeat t)
	       (case-fold t)
	       (tab-stop . nil)
